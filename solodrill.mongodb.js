// 🥋 JiuJitsu MongoDB Category Extractor 🥋
// Save your fingers for grips, not typing! 🤜✨
...
// Switch to our Jiujitsu database
use('Jiujitsu');

// 🤼‍♂️ Which category are we going to tackle today?
const categoryToExtract = "Takedowns"; 

// 🥷 Sneaky function to extract just what we need
function extractAndSaveCategory(data, categoryName) {
  // 🕵️‍♂️ Check if category exists
  if (!data[categoryName]) {
    print(`🚫 Whoops! "${categoryName}" category not found. Did someone pull guard on your data? 🤷‍♂️`);
    return false;
  }
  
  // 🧠 Extract just the techniques from this category
  const techniques = data[categoryName];
  
  // 📊 Convert the object to an array of documents
  const techniquesArray = Object.entries(techniques).map(([name, details]) => {
    return {
      name: name,
      ...
      related_techniques: details.related_techniques
    };
  });
  
  // 💾 First drop any existing collection with this name
  db[categoryName].drop();
  
  // 🚀 Insert all the techniques
  db[categoryName].insertMany(techniquesArray);
  
  print(`🎉 Successfully saved ${techniquesArray.length} techniques to the "${categoryName}" collection!`);
  print(`🔥 Your Jiujitsu database is getting stronger... unlike your opponent's guard! 💪`);
  
  return true;
}

// 🏃‍♂️ Let's execute our plan!
extractAndSaveCategory(basic_curriculum, categoryToExtract);

// 👀 Let's see what we've got
print("\n🔍 Here's a peek at what we saved:");
db[categoryToExtract].find().limit(3).pretty();
